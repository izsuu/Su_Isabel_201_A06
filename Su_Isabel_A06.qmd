...
[https://github.com/izsuu/Su_Isabel_201_A06](https://github.com/izsuu/Su_Isabel_201_A06)
...
```{r}
LJ_points1 <- player_info[[2]][[4]]
LJ_points1

LJ_points2 <-player_info$LJ$points
LJ_points2
```
...
for (i in 1:length(player_info)){ # iterates through each player
  ppg <- player_info[[i]][[4]]/player_info[[i]][[3]] # calculates points per game for each player
  player_initial <- names(player_info[i]) # gives initial of player
  points_per_game[[player_initial]] <- ppg
}
points_per_game
```
...
```{r}
round_points <- function(x){
  return(round(x, 2))
}

points_vector <- sapply(points_per_game, round_points)
points_vector
```
...
new_mat_rand <- apply(mat_rand, MARGIN = 1:2, round_mat)
new_mat_rand
```
f
## Question 4 (9 points)
...
```{r}
temp_av <- apply(max_temps[, 3:14], 2, mean)
temp_av
```
...
```{r}
CO_data <- max_temps[max_temps[, 1] == 'CO', ]
OH_data <- max_temps[max_temps[, 1] == 'OH', ]
VA_data <- max_temps[max_temps[, 1] == 'VA', ]
```
...
```{r}
CO_medians <- apply(CO_data[,3:14], 1, median)
CO_medians
which.max(CO_medians) # gives row # of highest median
which.min(CO_medians) # gives row # of lowest median

print(paste0('The maximum median annual temperature of ', CO_medians[which.max(CO_medians)], ' occured in ', CO_data[which.max(CO_medians), 2], ' and the minimum median annual temperature of ', CO_medians[which.min(CO_medians)], ' occurred in ', CO_data[which.min(CO_medians), 2]))
```
...
digit_sum <- function(num){
  if (num == 0){
    ...
    last_digit <- num %% 10
    trunc_num <- num %/% 10
    return(last_digit + digit_sum(trunc_num))
  }}
